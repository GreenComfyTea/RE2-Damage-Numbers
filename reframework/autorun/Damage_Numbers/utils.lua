...
function this.vec2.random(distance, min_direction, max_direction)
	...
	min_direction = min_direction or 0;
	max_direction = max_direction or 360;

	if min_direction > max_direction then
		local temp = min_direction;
		min_direction = max_direction;
		max_direction = temp;
	end

	min_direction = min_direction / 360;
	max_direction = max_direction / 360;

	local radians = ((max_direction - min_direction) * math.random() + min_direction) * math.pi * 2;
